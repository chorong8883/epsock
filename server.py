...
def recv_callback(fileno, recv_bytes):
    if not fileno in count:
        count[fileno] = 0
    ...
    while is_start and is_len and is_closer:
        ...
        if is_start and is_len and is_closer:
            message_bytes:bytes = recv_data[fileno][:end_index+len(closer)]
            server.send(fileno, message_bytes)
...
if __name__ == '__main__':
    ...
    server.start('218.55.118.203', 59012, count_thread=8, recv_callback=recv_callback)
    print("Server Start.")
